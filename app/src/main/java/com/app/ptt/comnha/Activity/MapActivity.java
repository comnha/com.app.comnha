...
public class MapActivity extends AppCompatActivity {
    ...
    public void showToast(final String a) {
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Toast.makeText(MapActivity.this, a, Toast.LENGTH_LONG).show();
            }
        });
    }
    ...
    public void loadData() {
        ...
        if (FRAGMENT_CODE.equals(getString(R.string.frag_map_CODE))) {
            ...
            if (findViewById(R.id.frame_map_adapter) != null) {
                if (getSupportFragmentManager().findFragmentById(R.id.frame_map_adapter) == null) {
                    try{
                        openMap(a);
                    } catch (Exception e){
